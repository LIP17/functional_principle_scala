...
class Empty extends TweetSet {
  ...
  def mostRetweeted: Tweet = throw new java.util.NoSuchElementException("Get Most Retweet on Empty Set")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    val other: TweetSet = left.filterAcc(p, right.filterAcc(p, acc))
    if(p(elem)) other.incl(elem) else other
  }

  def union(that: TweetSet): TweetSet = left.union(right.union(that.incl(elem)))

  def mostRetweeted: Tweet = {
    val more = left.filter(x => x.retweets > elem.retweets).union(right.filter(x => x.retweets > elem.retweets))
    try{
      more.mostRetweeted
    }catch {
